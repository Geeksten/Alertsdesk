...
class Staterecall(db.Model):
    ...
    state_recallid = db.Column(db.String(20), primary_key=True)
...
class Fdarecall(db.Model):
    ...
    def __repr__(self):
        ...
        return "<FDArecall recall_id=%s status=%s recall_date=%s recall_firm =%s prod_type=%s prod_desc=%s dist_pattern=%s recall_city=%s recall_state=%s recall_country=%s recall_reason=%s date_reported=%s>" % (
            self.recall_id, self.status, self.recall_date, self.recall_firm, self.prod_type, self.prod_desc, self.dist_pattern, self.recall_city, self.recall_state, self.recall_country, self.recall_reason, self.date_reported)
